...
| Домен                      | Что                         | Языки      |
| -------------------------- | --------------------------- | ---------- |
| `charva-travel.com`        | страница выбора направления | RU, EN, TM |
| `global.charva-travel.com` | туризм по Туркменистану     | RU, EN     |
...
Турецкий **снят, а не удалён** — решение владельца от 2026-09-11 (Q-17): в Stolzl нет `ğ`, `ı`
и `İ`, а подставлять системный шрифт на трёх самых частых буквах языка нельзя. Обе `tr.json`,
турецкие заголовки в `seo.ts` и турецкий контент в базе на месте; возврат — одна строка в
`SITE_LANGS`. До тех пор `/tr/…` отдаёт 302 на тот же адрес по-русски.
