 ...
 * two ways forward that actually help — the programme and the signup form — rather than an
 * apology.
 */
      ...
      <Container>
        <div className="mx-auto flex max-w-[560px] flex-col items-center gap-6 py-24 text-center">
          ...
          <div className="mt-2 flex flex-wrap justify-center gap-3">
            ...
            <Link to={path.maksatnama(lang)} className={buttonClass({ variant: 'outline' })}>
              {copy.nav.items.maksatnama}
            </Link>
            <Link to={path.yazylmak(lang)} className={buttonClass({ variant: 'outline' })}>
              {copy.nav.cta}
            </Link>
          </div>
        </div>
