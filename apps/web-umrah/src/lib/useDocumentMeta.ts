...
export function useDocumentMeta(
  ...
  useEffect(() => {
    ...
    for (const code of [...SITE_LANGS.umrah, 'x-default'] as const) {
      const link = document.createElement('link');
      link.rel = 'alternate';
      link.hreflang = code;
      // `x-default` points at Turkmen: it is this site's default language, not a third one.
      link.href = new URL(
        `/${code === 'x-default' ? 'tm' : code}${pathAfterLang}`,
        location.origin,
      ).toString();
      link.dataset['charva'] = '';
