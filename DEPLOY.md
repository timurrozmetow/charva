...
| Хост    | `charva-travel.com`, 159.200.240.75 | —                                                   |
...
Имя файла в кеше — хеш от **ключа, ширины и настроек кодировщика** (`DERIVATIVE_TAG` в
`apps/api/src/modules/media/routes.ts`). Поэтому смена качества или `effort` не требует ничего
чистить: новые URL дают новые имена, а старые файлы становятся недостижимы и уезжают по `atime`
той же строкой крона выше. До того как тег попал в имя, смена настроек не меняла **ничего** —
на диске находился прежний файл и отдавался ещё год.
